    ...
    <div className="bg-zinc-950 min-h-screen py-10 px-4 sm:px-6 md:px-12 lg:px-20 text-white">
      <div className="max-w-6xl w-full mx-auto space-y-6">
        ...
        <motion.section
          ...
          className="relative overflow-hidden w-full rounded-2xl border border-emerald-400/20 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 px-4 py-5 sm:px-6 sm:py-6 mb-5 shadow-[0_0_35px_rgba(52,211,153,0.06)]"
        >
          <div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-emerald-400/10 blur-3xl" />
          <div className="pointer-events-none absolute -left-24 bottom-0 h-48 w-48 rounded-full bg-cyan-500/10 blur-3xl" />
              ...
              {[...cpHistory]
                ...
                .map((ev: any, i: number) => {
                  ...
                  if (ev.type === 'usd') {
                    ...
                    detail = ev.channel ? `${String(ev.channel).toUpperCase()} share` : 'Share activity';
                  } else if (ev.type === 'deadcoin_first') {
                    detail = ev.token_contract ? `Contract: ${shorten(ev.token_contract)}` : 'First deadcoin signal';
                  } else if (ev.type === 'referral_signup') {
                    detail = ev.ref_wallet ? `Referee: ${shorten(ev.ref_wallet)}` : 'New referred wallet';
                  } else {
                    detail = ev.detail || '-';
                  }
