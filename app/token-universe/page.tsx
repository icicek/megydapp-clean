...
function getClassificationLabel(status: TokenStatus, reason: string | null) {
    if (status === 'redlist' || status === 'blacklist') return 'Policy Override';

    const r = String(reason || '').toLowerCase();

    if (r.includes('metrics_reclassification') || r.includes('reclass')) {
        return 'Auto Metrics';
    }
                                    ...
                                    <tr key={it.mint} className="border-t border-white/5">
                                        ...
                                        <td className="p-3">
                                            <span className="inline-flex rounded px-2 py-1 text-xs border border-white/10 bg-white/5 text-gray-200">
                                                {getClassificationLabel(it.status, it.reason)}
                                            </span>
                                        </td>
