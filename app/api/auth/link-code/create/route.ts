//app/api/auth/link-code/create/route.ts
import { NextResponse } from 'next/server';
import { cookies } from 'next/headers';
import { sql } from '@/app/api/_lib/db';
import { USER_AUTH_COOKIE, verifyUserSession } from '@/app/api/_lib/user-auth';

export const dynamic = 'force-dynamic';

function createIdentityLinkCode() {
  const n = Math.floor(100000 + Math.random() * 900000);
  return `MEGY-${n}`;
}

export async function POST() {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get(USER_AUTH_COOKIE)?.value;

    if (!token) {
      return NextResponse.json(
        { ok: false, error: 'Identity session required.' },
        { status: 401 }
      );
    }

    const session = verifyUserSession(token);

    if (!session) {
      return NextResponse.json(
        { ok: false, error: 'Invalid identity session.' },
        { status: 401 }
      );
    }

    const identityRows = await sql`
      SELECT id
      FROM identities
      WHERE id = ${session.identityId}
        AND status = 'active'
      LIMIT 1
    `;

    if (identityRows.length === 0) {
      return NextResponse.json(
        { ok: false, error: 'Active identity not found.' },
        { status: 404 }
      );
    }

    await sql`
      UPDATE identity_link_codes
      SET used_at = NOW()
      WHERE identity_id = ${session.identityId}
        AND purpose = 'link_wallet'
        AND used_at IS NULL
        AND expires_at > NOW()
    `;

    let code = createIdentityLinkCode();

    for (let i = 0; i < 5; i += 1) {
      try {
        const rows = await sql`
          INSERT INTO identity_link_codes (
            identity_id,
            code,
            purpose,
            expires_at
          )
          VALUES (
            ${session.identityId},
            ${code},
            'link_wallet',
            NOW() + INTERVAL '15 minutes'
          )
          RETURNING code, expires_at
        `;

        return NextResponse.json({
          ok: true,
          code: rows[0].code,
          expiresAt: rows[0].expires_at,
        });
      } catch {
        code = createIdentityLinkCode();
      }
    }

    return NextResponse.json(
      { ok: false, error: 'Failed to create a unique link code.' },
      { status: 500 }
    );
  } catch (error) {
    console.error('[auth/link-code/create] error:', error);

    return NextResponse.json(
      { ok: false, error: 'Failed to create identity link code.' },
      { status: 500 }
    );
  }
}