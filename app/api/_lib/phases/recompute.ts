...
export async function recomputeFromPhaseId(phaseId: number): Promise<RecomputeResult> {
  ...
  const lockKey =
    (BigInt(942001) * BigInt(1_000_000_000) + BigInt(Math.trunc(phaseId))).toString();
  ...
  try {
    ...
    try {
      ...
      if (!startRow) {
        throw Object.assign(new Error('PHASE_NOT_FOUND'), {
          statusCode: 404,
      ...
      if (!phases.length) {
        throw Object.assign(new Error('NO_PHASES_TO_RECOMPUTE'), {
          statusCode: 409,
          code: 'NO_PHASES_TO_RECOMPUTE',
        });
      }
