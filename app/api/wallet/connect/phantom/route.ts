// app/api/wallet/connect/phantom/route.ts
import { NextResponse } from 'next/server';
import { assertAllowedRedirect } from '@/app/api/_lib/url';

// GET /api/wallet/connect/phantom?redirect_link=...&app_url=...&dapp_encryption_public_key=...
export async function GET(req: Request) {
  const { searchParams } = new URL(req.url);
  const redirect = searchParams.get('redirect_link');
  const appUrl   = searchParams.get('app_url') || 'https://coincarnation.com';
  const dappPub  = searchParams.get('dapp_encryption_public_key'); // base58/hex
  ...
  try {
    ...
    // ul.searchParams.set('cluster', 'mainnet-beta'); // istersen ekle
